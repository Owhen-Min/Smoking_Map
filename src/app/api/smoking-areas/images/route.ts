...
export async function POST(request: Request) {
  try {
    const { smoking_area_id, image_url_hd, image_url_sd, is_primary } =
      await request.json();
    ...
    if (!smoking_area_id || !image_url_hd || !image_url_sd) {
      return NextResponse.json(
        {
          error:
            "필수 필드가 누락되었습니다. (smoking_area_id, image_url_hd, image_url_sd)",
        },
        { status: 400 },
      );
    }
    ...
    if (!supabaseUrl) {
      return NextResponse.json(
        {
          error:
            "서버 환경 변수(NEXT_PUBLIC_SUPABASE_URL)가 설정되지 않았습니다.",
        },
        { status: 500 },
      );
    }
    ...
    if (
      !image_url_hd.startsWith(allowedPrefix) ||
      !image_url_sd.startsWith(allowedPrefix)
    ) {
      return NextResponse.json(
        {
          error:
            "허용되지 않은 이미지 URL 경로입니다. 우리 서비스의 스토리지 경로만 허용됩니다.",
        },
        { status: 400 },
      );
    }
